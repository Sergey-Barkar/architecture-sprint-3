...
Предварительно сконфигурируйте keycloak:<br/>
1. Дождитесь старта keycloak;<br/>
2. Зайдите в админку. Порт 18080. Логин admin, пароль аналогичный;<br/>
3. Создайте рилм, это должен быть "shsmvp";<br/>
4. Создайте юзера, либо в параметрах рилма включите флаг отвечающий за взможность регистрации;<br/>
5. Создайте клиент для данного рилма.<br/>
В пункте "Valid redirect URIs" укажите любые пути от сервисов телемитрии и устройств пользователя (http://localhost:8080/* и http://localhost:8081/*).<br/>
В пункте "Web origins" укажите URL'ы сервисов телемитрии и устройств пользователя (http://localhost:8080 и http://localhost:8081).<br/>
Включите Implicit flow. <br/>
Swagger доступен по адресам:<br/>
http://localhost:8080/swagger/index.html - устройства пользователя<br/>
http://localhost:8081/swagger/index.html - телеметрия<br/>
Работоспособность кафки можно проверить через функцию action сервиса устройств. В ходе её работы должна осуществляться коммуникация с сервисом "коммуникации" (реализация которого выходит за рамки данной задачи), результат коммуникации должен записываться обратно в Кафку, что и сделано, с оговоркой - телеметрия синтетическая.<br/>
Соответственно по итогу работы функции action в сервисе телеметрии появляются новые данные для специфических устройств.<br/>
